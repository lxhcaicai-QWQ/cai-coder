...
def test_http_post():
    ...
    result = http_post("https://httpbin.org/post", json_data=test_data)
    assert result["success"] is True
    assert result["status_code"] == 200
...
def test_http_with_params():
    ...
    result = http_get("https://httpbin.org/get", params=params)
    assert result["success"] is True
    assert result["status_code"] == 200
...
def test_http_error_handling():
    """测试错误处理"""
    # 测试不存在的 URL
    result = http_get("https://nonexistent-domain-12345.com")
...
def test_http_404_error():
    """测试 404 错误"""
    result = http_get("https://httpbin.org/status/404")
